function get_sheet_main_values (spreadsheet, vueobj, sheet_id_map) {
  const data = [];
  ...
  vueobj.serialized_data.dealbasicinfo.weblinks.forEach(function (x) {
    cur_row = cur_row + 1
    data.push({
      range: 'Main!B' + cur_row.toString() + ':B' + cur_row.toString(),
  ...
  cur_row = cur_row + 1
  data.push({
    range: 'Main!A' + cur_row.toString() + ':B' + cur_row.toString(),
...
function get_sheet_main (spreadsheet, vueobj, sheet_id_map) {
  const requests = [];

  requests.push({
    "repeatCell": {
      "range": {
        "sheetId": sheet_id_map.Main,
        "startRowIndex": 0,
...
export default {
  get_sheet_main_values: get_sheet_main_values,
  get_sheet_main: get_sheet_main
}
